...
function paragrafOlustur(cumleler, cumleKur, cumlelereDonustur) {
  let ayrac = " "
  let yeniDizi = [];
  yeniDizi = cumlelereDonustur(cumleler, ayrac);
  let paragraf = cumleKur(yeniDizi[1], yeniDizi[3], yeniDizi[5], yeniDizi[7], yeniDizi[9]);
  return paragraf;
}
...
for(let key in emojiler) {
  atilanMesaj = atilanMesaj.replaceAll(key.toUpperCase(), emojiler[key]);
  atilanMesaj = atilanMesaj.replaceAll(key, emojiler[key]);
}
return atilanMesaj;
}
console.log(emojileriDonustur("Merhaba, nasılsınız :p", emojiler));
